...
int main(int argc, char* argv[]) {
	...
	// Solve
	performance->solve();
	performance::ThrottlingPerformance* throttlingPerformance = new performance::ThrottlingPerformance(performance, 1.0);
	...
	double r1 = 1.2;				// Throttle value initial value
	double r2 = 0.5;				// Throttle value final value

	for (int i=0; i<=n; ++i) {

		double pa = p1 + (p2-p1)*(double)i/(double)n;				// Ambient pressure current value

		double throttleValue = r1 + (r2-r1)*(double)i/(double)n;	// Throttle value current value
		double mdot = throttleValue*nominalMdot;

		// Re-calculate throttle performance
		throttlingPerformance->throttle(throttleValue);

		// Get nozzle exit object
		performance::equilibrium::NozzleSectionConditions* nozzleExit = (throttlingPerformance)->getExitSection();
		performance::equilibrium::NozzleSectionConditions* flowSeparationSection = getFlowSeparation(throttlingPerformance, pa, checkFlowSeparation);
